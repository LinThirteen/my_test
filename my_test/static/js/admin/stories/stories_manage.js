$(function () {
  ...
  // 删除标签
  let $storiesDel = $(".btn-del");  // 1. 获取删除按钮
  $storiesDel.click(function () {   // 2. 点击触发事件
    let _this = this;
    let storiesId = $(this).data('stories-id');
    swal({
      title: "确定删除这篇小说吗?",
      text: "删除之后，将无法恢复！",
      type: "warning",
      ...
      $.ajax({
        // 请求地址
        url: "/admin/stories/" + storiesId + "/",  // url尾部需要添加/
        // 请求方式
        type: "DELETE",
        dataType: "json",
      })
  ...
  let $title = $("#title");
  let $author = $("#author");
  $cleans.click(function () {   // 2. 点击触发事件
     $title.val('');
     $author.val('');
     $startTime.val('');
     $endTime.val('');
